...
public final class Base64Encoder {
	...
	public static String encodeUrl(byte[] value) {
		...
		for (int i = 0; i < value.length; i++) {
			...
			if (bufferIndex == 3 || i == (value.length - 1)) {
				// Make a first character
				base64CharIndex = buffer[0] >>> 2;
				output[outputIndex++] = BASE64__URL_CHARS.charAt(base64CharIndex);
				...
				// Make a third character
				base64CharIndex = ((buffer[1] & 0x0F) << 2) | (buffer[2] >>> 6);
				output[outputIndex++] = bufferIndex > 1 ? BASE64__URL_CHARS
						.charAt(base64CharIndex) : BASE64_PADDING;

				// Make a fourth character
				base64CharIndex = buffer[2] & 0x3F;
				output[outputIndex++] = bufferIndex > 2 ? BASE64__URL_CHARS
						.charAt(base64CharIndex) : BASE64_PADDING;
	...
	public static byte[] decodeUrl(String value) throws Exception {
		if (value == null || value.length() == 0)
			return new byte[0];
		...
		for (int i = 0; i < value.length(); i++) {
			...
			// If it is a Base64 character
			if (BASE64__URL_CHARS.indexOf(chr) != -1)
				continue;
		...
		for (int i = 0; i < value.length(); i++) {
			...
			if (bufferIndex == 4) {
				// Make a first byte
				base64CharIndex = BASE64__URL_CHARS.indexOf(buffer[0]);
				output[outputIndex] = (byte) (base64CharIndex << 2);

				base64CharIndex = BASE64__URL_CHARS.indexOf(buffer[1]);
				output[outputIndex++] |= base64CharIndex >>> 4;
				...
				base64CharIndex = BASE64__URL_CHARS.indexOf(buffer[2]);
				output[outputIndex++] |= base64CharIndex >>> 2;
				...
				base64CharIndex = BASE64__URL_CHARS.indexOf(buffer[3]);
				output[outputIndex++] |= base64CharIndex;
